...
// 获取精练属性
function getRefineMeta() {
    window.refineMeta = {}
    const rList = window.meta.getAllRefineMeta();
    rList.forEach(item => {
        window.refineMeta[item.id] = {
            name: item.name,
            info: item.info
        };
        parent.addRefineToList(item.id, item.name);
    });
}
...
function getEquipInfo() {
    window.equipInfo = {}
    const eList = window.meta.getAllEquipMeta();
    eList.forEach(item => {
        window.equipInfo[item.id] = {
            name: item.name,
            info: item.info
        };
        parent.addEquipToList(item.id, item.name);
    });
}
...
function getHeroInfo() {
    window.heroInfo = {}
    const hList = window.me.userInfo.heroArray;
    parent.clearHeroList();
    hList.forEach(item => {
        parent.addHeroToList(item.id, `角色${item.id + 1}`);
    });
}
...
// 自动精炼装备触发事件
function autoRefineEvent() {
    // contentFrame.contentWindow.kinkoo_jinglian.reqRefineSkill(equipId, heroId, equipList.selectedIndex + 1, window.indexId, 1, 0); // 自动精炼装备
    window.kinkoo_jinglian.reqRefineSkill(
        window.refineManager.equipId,
        window.refineManager.heroId,
        window.refineManager.selectedIndex,
        window.refineManager.indexId, 1, 0); // 自动精炼装备
}
...
function _messageBuff_receiveMessage(msg, params) {
    ...
    // 精练
    if (msg == "EquipRefineAck") {
        // readyRefines, 装备的孔位数组
        // params.change.equips[0].readyRefines
        if (window.refineManager) {

            const refineId = params.change.equips[0].readyRefines[window.refineManager.indexId];
            const refineMeta = window.refineMeta[refineId];

            if (refineId == window.refineManager.refineId) {
                console.warn('装备:', params.change.equips[0].id, '成功精练到:', refineMeta);
                window.refineManager.autoRefine = false
                parent.alert('装备成功精练到:' + refineMeta.name)
            } else {
                console.warn('精练结果:', params.change.equips[0].id, refineMeta);
                if (window.refineManager.autoRefine) {
                    autoRefineEvent();
                }
            }
        }
